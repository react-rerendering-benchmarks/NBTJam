...
function App({
  ...
  const currentFilename = useRef<null | string>(null);
  const [currentModifyVersion, setCurrentModifyVersion] = useState<number>(0);
  const selectedNode = useRef<null | NBTDataNode>(null);
  const [loading, setLoading] = useState<string>('');
  bus.updateCurrentFileUUIDSetter([currentFileUUID, setCurrentFileUUID]);
  bus.updateCurrentFilenameSetter([currentFilename.current, setCurrentFilename]);
  bus.updateCurrentModifiedSetter([currentModifyVersion, setCurrentModifyVersion]);
  bus.updateSelectedNodeSetter([selectedNode.current, setSelectedNode]);
