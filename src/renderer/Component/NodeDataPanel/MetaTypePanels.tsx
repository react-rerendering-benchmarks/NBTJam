...
function integerInputNumber(type: 'byte' | 'short' | 'int', min: number, max: number): MetaTypePanelValueInputFun {
  return () => <InputNumber min={min} max={max} style={{
    width: '100%'
...
e.long = metaTypePanel({
  ...
  valueInput: ({
    ...
    const value = useRef('');
    useEffect(() => {
      value.current.value = form.getFieldValue('value');
    }, [form.getFieldValue('value')]);
    return <Input ref={value} />;
  }
});
e.string = metaTypePanel({
  ...
  valueInput: ({
    ...
    }} allowClear={true} maxLength={65535} autoSize={{
      maxRows: 10
    }} onChange={e => {
