...
const renderNode = (node: NBTDataNode) => {
  ...
  const prefixStr = ellipsisMiddle(typeof node.keyInParent === 'string' ? (keyInParent as string) : pad3((keyInParent as number)), 20);
  const prefix = typeof node.keyInParent === 'string' ? <>{prefixStr}: </> : <Tag color="purple">{prefixStr}</Tag>;
  let title: typeof node.title | string;
  if (node.dummyMoreNode) {
    title = <span style={{
      color: '#ccc'
    }}>We shows at most {MAX_CHILDREN_COUNT} children.</span>;
  } else if (node.isLeaf) {
    ...
    switch (node.valueType) {
      ...
      case 'int_array':
        {
          const unit = node.valueType === 'compound' ? `entr${node.realChildrenCount !== 1 ? 'ies' : 'y'}` : `element${node.realChildrenCount !== 1 ? 's' : ''}`;
      ...
      case 'list':
        {
          ...
          title = <>
            <span style={{
              color: '#b0b0b0'
            }}>[]</span> <Tag color="default" style={{
              marginRight: '0'
            }}>{node.realChildrenCount} {unit}</Tag>
  ...
  return <span aria-label={(node.key as string)}>
      {prefix}{title}
    </span>;
};
function NBTEditorTree({
  ...
  const expandedNodes = useRef<NBTDataNode[]>([]);
  useEffect(() => {
    const data: DataNode[] = [];
    const {
      root
  ...
  function resetTreeWidth(wrapper: HTMLDivElement) {
    ...
    let maxWidth = 0;
    for (const node of expandedNodes.current) {
      if (node.width && node.width > maxWidth) maxWidth = node.width;
    }
    if (!maxWidth) return;
    holder.style.width = `${maxWidth}px`;
  }
  return <div ref={ref} id="nbt-tree">
      <Tree showLine={{
      showLeafIcon: false
    }} showIcon={true} treeData={treeData} virtual={true} height={height} titleRender={renderNode} selectedKeys={[bus.selectedNode?.key]} onSelect={(selectedKeys, info) => {
      if (!info.selected) {
        bus.selectedNode = null;
      } else {
      ...
      expandedNodes.current = uniq(ret);
    }} />
      <div style={{
      display: 'none'
    }}>{currentModifyVersion}</div>
    </div>;
}
